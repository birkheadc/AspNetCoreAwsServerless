...
public class SessionControllerTests
{
  ...
  public async Task Login_ReturnsFailureResult_WhenLoginFails()
  {
    ...
    _sessionService
      ...
      .ReturnsAsync(ApiResult<SessionContext>.BadRequest);
  ...
  public async Task Login_SignsInUser_AndReturnsSuccessResult_WhenLoginSucceeds()
  {
    ...
    SessionContext sessionContext =
      new() { User = user, Tokens = _mocker.CreateInstance<SessionTokens>() };

    UserDto expected = _mocker.CreateInstance<UserDto>();

    _sessionService
      .Setup(x => x.Login(It.IsAny<LoginDto>()))
      .ReturnsAsync(ApiResult<SessionContext>.Success(sessionContext));
